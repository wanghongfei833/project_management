...
class ProjectAdjustmentApproval(db.Model):
    """项目应收追加的审批记录。"""
    __tablename__ = "project_adjustment_approvals"

    id = db.Column(db.Integer, primary_key=True)
    adjustment_id = db.Column(
        db.Integer,
        db.ForeignKey("project_expected_income_adjustments.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    adjustment = db.relationship("ProjectExpectedIncomeAdjustment", back_populates="approvals")
...
class ProjectExpectedIncomeAdjustment(db.Model):
    ...
    note = db.Column(db.Text, nullable=True)
    status = db.Column(db.String(16), nullable=False, default="active")  # active/pending/executed/cancelled
    created_by_user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)

    project = db.relationship("Project", back_populates="adjustments")
    approvals = db.relationship("ProjectAdjustmentApproval", back_populates="adjustment")
...
class ProjectEndRequest(db.Model):
    """项目终止申请。"""
    __tablename__ = "project_end_requests"
    ...
    project = db.relationship("Project")
    approvals = db.relationship("ProjectEndApproval", back_populates="request")


class ProjectEndApproval(db.Model):
    """项目终止的审批记录。"""
    __tablename__ = "project_end_approvals"

    request_id = db.Column(
        db.Integer,
        db.ForeignKey("project_end_requests.id", ondelete="CASCADE"),
        primary_key=True,
    )
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), primary_key=True)
    approved_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)

    request = db.relationship("ProjectEndRequest", back_populates="approvals")
    user = db.relationship("User")


class ProjectReviveRequest(db.Model):
    """项目复活申请。"""
    __tablename__ = "project_revive_requests"
    ...
    project = db.relationship("Project")
    approvals = db.relationship("ProjectReviveApproval", back_populates="request")


class ProjectReviveApproval(db.Model):
    """项目复活的审批记录。"""
    __tablename__ = "project_revive_approvals"

    request_id = db.Column(
        db.Integer,
        db.ForeignKey("project_revive_requests.id", ondelete="CASCADE"),
        primary_key=True,
    )
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), primary_key=True)
    approved_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)

    request = db.relationship("ProjectReviveRequest", back_populates="approvals")
    user = db.relationship("User")
