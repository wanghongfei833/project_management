...
def ensure_sqlite_schema():
    ...
    if not _table_exists("project_adjustment_approvals"):
        db.session.execute(
            text(
                """
                CREATE TABLE project_adjustment_approvals (
                  id INTEGER PRIMARY KEY,
                  adjustment_id INTEGER NOT NULL,
                  user_id INTEGER NOT NULL,
                  approved_at DATETIME NOT NULL,
                  FOREIGN KEY(adjustment_id) REFERENCES project_expected_income_adjustments(id) ON DELETE CASCADE,
                  FOREIGN KEY(user_id) REFERENCES users(id)
                )
                """
            )
        ...
        db.session.execute(
            text(
                "CREATE INDEX ix_project_adjustment_approvals_adjustment_id "
                "ON project_adjustment_approvals (adjustment_id)"
            )
        )

    # Add status column to project_expected_income_adjustments
    if _table_exists("project_expected_income_adjustments") and not _column_exists("project_expected_income_adjustments", "status"):
        db.session.execute(
            text("ALTER TABLE project_expected_income_adjustments ADD COLUMN status VARCHAR(16) NOT NULL DEFAULT 'active'")
        )

    # Project end-request workflow
    if not _table_exists("project_end_requests"):
        db.session.execute(
            text(
                """
                CREATE TABLE project_end_requests (
                  id INTEGER PRIMARY KEY,
                  project_id INTEGER NOT NULL,
                  status VARCHAR(16) NOT NULL DEFAULT 'open',
                  created_by_user_id INTEGER,
        ...
        db.session.execute(
            text("CREATE INDEX ix_project_end_requests_project_id ON project_end_requests (project_id)")
        )

    if not _table_exists("project_end_approvals"):
        db.session.execute(
            text(
                """
                CREATE TABLE project_end_approvals (
                  request_id INTEGER NOT NULL,
                  user_id INTEGER NOT NULL,
                  approved_at DATETIME NOT NULL,
                  PRIMARY KEY (request_id, user_id),
                  FOREIGN KEY(request_id) REFERENCES project_end_requests(id) ON DELETE CASCADE,
                  FOREIGN KEY(user_id) REFERENCES users(id)
                )
                """
            )
    ...
    # Add ended_at to projects
    if _table_exists("projects") and not _column_exists("projects", "ended_at"):
        db.session.execute(
            text("ALTER TABLE projects ADD COLUMN ended_at DATETIME")
        )

    # Project revive workflow
    if not _table_exists("project_revive_requests"):
        db.session.execute(
            text(
                """
                CREATE TABLE project_revive_requests (
                  id INTEGER PRIMARY KEY,
                  project_id INTEGER NOT NULL,
                  status VARCHAR(16) NOT NULL DEFAULT 'open',
                  created_by_user_id INTEGER,
        ...
        db.session.execute(
            text("CREATE INDEX ix_project_revive_requests_project_id ON project_revive_requests (project_id)")
        )

    if not _table_exists("project_revive_approvals"):
        db.session.execute(
            text(
                """
                CREATE TABLE project_revive_approvals (
                  request_id INTEGER NOT NULL,
                  user_id INTEGER NOT NULL,
                  approved_at DATETIME NOT NULL,
                  PRIMARY KEY (request_id, user_id),
                  FOREIGN KEY(request_id) REFERENCES project_revive_requests(id) ON DELETE CASCADE,
                  FOREIGN KEY(user_id) REFERENCES users(id)
                )
                """
            )
